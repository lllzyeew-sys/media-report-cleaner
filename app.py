...
if uploaded:
    try:
        ...
        if not cols["title"] or not cols["hit"]:
            ...
            with r1:
                st.download_button("下载 Americas Keep CSV", to_csv_bytes(keep_americas), f"{base}_keep_americas.csv", "text/csv")
            with r2:
                st.download_button("下载 JP Keep CSV", to_csv_bytes(keep_jp), f"{base}_keep_jp.csv", "text/csv")
            with r3:
                st.download_button("下载 EU Keep CSV", to_csv_bytes(keep_eu), f"{base}_keep_eu.csv", "text/csv")
            with r4:
                st.download_button("下载 APAC excl. JP Keep CSV", to_csv_bytes(keep_apac), f"{base}_keep_apac_exclude_jp.csv", "text/csv")
            with r5:
                st.download_button("下载 Other / Unassigned CSV", to_csv_bytes(keep_other), f"{base}_keep_other_unassigned.csv", "text/csv")
