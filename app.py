...
APP_VERSION = "v5.0 - English UI + region downloads + stronger executive scanning"
...
if uploaded:
    try:
        ...
        if not cols["title"] or not cols["hit"]:
            ...
            review = classified[classified["Decision"] == "Review"]
            with c1:
                st.download_button("Download Keep CSV", to_csv_bytes(keep), f"{base}_keep.csv", "text/csv")
            with c2:
                st.download_button("Download Removed CSV", to_csv_bytes(removed), f"{base}_removed.csv", "text/csv")
            with c3:
                st.download_button("Download Review CSV", to_csv_bytes(review), f"{base}_review.csv", "text/csv")
            with c4:
                st.download_button("Download All Classified CSV", to_csv_bytes(classified), f"{base}_classified.csv", "text/csv")
            ...
            with r1:
                st.download_button("Download Americas Keep CSV", to_csv_bytes(keep_americas), f"{base}_keep_americas.csv", "text/csv")
            with r2:
                st.download_button("Download JP Keep CSV", to_csv_bytes(keep_jp), f"{base}_keep_jp.csv", "text/csv")
            with r3:
                st.download_button("Download EU Keep CSV", to_csv_bytes(keep_eu), f"{base}_keep_eu.csv", "text/csv")
            with r4:
                st.download_button("Download APAC excl. JP Keep CSV", to_csv_bytes(keep_apac), f"{base}_keep_apac_exclude_jp.csv", "text/csv")
            with r5:
                st.download_button("Download Other / Unassigned CSV", to_csv_bytes(keep_other), f"{base}_keep_other_unassigned.csv", "text/csv")

            st.subheader("按地区Download Keep CSV")
            r1, r2, r3, r4, r5 = st.columns(5)
            keep_americas = keep[keep["Region Group"] == "americas"]
            keep_jp = keep[keep["Region Group"] == "jp"]
            keep_eu = keep[keep["Region Group"] == "eu"]
